...
## Notes
- TEST_REPORT.md round 2 (3485e0fd): WEAK_PASS. F1/F2 confirmed. F3 half-explained by builder: the nucleus-side trigger is `preflight._probe_mlx_lm_version()` importing `mlx_lm` (decisive experiment: a first test calling only that probe fails all 4 victims); why a fresh import elsewhere doesn't is unexplained and outside nucleus. Ticketed with recommended fix (`importlib.util.find_spec` + `importlib.metadata.version`, no import; also saves `nucleus plan` ~1.5 s). Note: `nucleus-tests.yml` runs only `tests/nucleus` with `ARAIL_NUCLEUS_STUB=1`; ARCHITECTURE §7's four-path list is not what CI runs — flagged for architect. Full-suite runs leave test-server processes behind (124 killed by QA).
