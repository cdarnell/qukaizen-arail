...
| build (QA loop) | builder | BUILD_LOG.md | done | 2026-09-22 01:00 | 2026-09-22 02:40 | 7 commits `61f34bd7`…`b81cd978`; all 5 must-fix + F3 judgment call; F1/F6/F10 filed as debt |
| re-test | qa | TEST_REPORT.md (appended) | done | 2026-09-22 02:45 | 2026-09-22 09:00 | **WEAK_PASS** (commit `bd16f383`) — 0 branch-only failures; F1/F6 xfail(strict) → debt; one partial (F7 UI) filed as debt |
| ship | — | PR | **shipped** | 2026-09-24 | 2026-09-24 | operator: "push and ship both" → branch pushed, [PR #196](https://github.com/cdarnell/qukaizen-arail/pull/196) open against main; W5 still open |
...
| 2026-09-22 | QA fix loop verified by the orchestrator | QA test files: 1312 passed, 6 xfailed, **2 failed** — identical across two runs; the two are F1 (`test_json_quoted_api_key_is_redacted_before_disk`) and F6 (`test_malformed_json_on_a_mutating_admin_endpoint_is_not_a_500`), both items QA itself filed as debt while also committing a test for them. `tests/test_recap_*`: 174 passed with no `ARAIL_DATA_DIR` set (was 23 failed). Real `lab/data` clean including `costs.json`, not recreated. Builder's whole-tree differential vs main, run twice byte-identical: fail-both 40, only-main 1, only-branch 4 = F1 + F6 + two `test_qa_security_surface.py` tests traced to a pre-existing daemon-thread leak (TEST_REPORT F11) — those two did not fail in the orchestrator's QA-only runs, so they are order/load-dependent. |
| 2026-09-22 | F3 fixed rather than filed (builder judgment call, accepted) | One-line `isinstance` guard; a 500 on every Admin page load from a single corrupt log line is a setup-tier defect on a lab that runs on someone else's machine. |
| 2026-09-22 | F1/F6 red-test tension routed to QA, not resolved by the orchestrator | The orchestrator's instruction asked for both "file F1/F6 as debt" and "every `test_qa_*` green" — contradictory as written; the builder surfaced it instead of picking silently. QA owns its tests: either mark them `xfail(strict=True)` referencing the BACKLOG entry (the ingress-spine pattern — loud, cannot be lost) or promote the finding to must-fix. |
| 2026-09-22 | QA re-test WEAK_PASS — test → ship gate passes | All five must-fix items satisfied at the code level, verified beyond the index (tmp cleanup; crash between tmp-write and `os.replace` leaves no misreadable partial; trace schema carries no free-text exception field; metadata record still written when redaction fails; F9's `DATA_DIR` binding proven test-only by an AST check that `config.DATA_DIR` is never rebound in `src`). W1–W4 pass; hot path p95 44.9/46.4 µs, unmoved. Whole-tree differential vs `main@236504ca`: fail-both 40, only-main 1, **only-branch 0**. QA's rulings: F1/F6 → debt via `xfail(strict=True)` naming their BACKLOG entries; the two load-dependent security tests hardened (root cause: `importlib.reload(arail.activity)` in a boot test leaves `app.py` and `arail.activity` holding different `ActivityLog` objects — test-harness, not product). **One partial filed as debt (re-test R1):** the purge fix is complete on the server but `admin.html:1839,1879` ignore the new `ok` flag and log a failed purge at `info` as "0 on disk, 0 in memory" — three lines of JS; not promoted because the legacy notice re-scans and still shows the true count. Also filed: R2 (a class-name-shaped string carrying a secret passes the `error_class` allow-list — no live producer), R3/R4 doc-vs-behaviour, R5 (`monkeypatch.undo()` reverts conftest fixtures). Orchestrator confirmed: QA files 1364 passed / 10 xfailed, real `lab/data` clean. |
| 2026-09-22 | Sprint totals | 67 commits on `qukaizen/arail-buddy-front-and-center` over main; 75 files, +18,342 / −219. Two rate-limit interruptions (builder, QA) recovered with no lost work. Ship awaits the operator's explicit confirmation (push + PR) and the W5 witness line. |
| 2026-09-24 | **Shipped on the operator's explicit instruction** ("push and ship both please") | `qukaizen/arail-buddy-front-and-center` pushed to origin; PR #196 opened against main with the pipeline verdicts, test evidence, and the filed debt in the description. The independent rename branch `qukaizen/arail-queuellm-local-rename` was pushed and opened as PR #195 in the same action. W5 remains the operator's to write. |
